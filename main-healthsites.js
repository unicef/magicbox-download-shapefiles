...
let country_codes = require('./country-names');
...
bluebird.each(Object.keys(country_codes), function(e) {
...
function download_shapefile_then_unzip(country_code) {
  return new Promise((resolve, reject) => {
    let url = 'https://healthsites.io/media/shapefiles/' + country_codes[country_code] + '_shapefile.zip'
  ...
  return new Promise(function(resolve, reject) {
    mkdirp(shapefile_dir + '/' + country_code, function(err) {
      ...
      extract(path, {dir: shapefile_dir + '/' + country_code}, error => {
        ...
        let geojsonPath = './data/geojson/' + country_code + '.json'
        let csvPath = './data/csv/' + country_code + '.csv'
        let command = 'ogr2ogr -f geoJSON ' + geojsonPath + ' ' + shapefile_dir + country_code +
        '/*.shp'
        ...
        const child = exec(command,
            (error, stdout, stderr) => {
                ...
                geojson2csv(geojsonPath, csvPath, function(err){
                  if(err) throw err
                  resolve()
                })
